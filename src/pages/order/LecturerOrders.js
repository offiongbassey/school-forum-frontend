...
import { getLecturerOrders } from '../../services/orderService';

const LecturerOrders = () => {
    ...
    useEffect(() => {
        ...
        async function getOrders(){
            const storeOrders =  await getLecturerOrders();
                            ...
                            <tbody>
                                {
                                  currentItmes.map((order, index) => {
                                        const {_id, productId, productCode, orderCode, grandTotal, status, transactionCode, createdAt, userId } = order;
                                        let newDate = new Date(createdAt);
                                        return (
                                            <tr key={_id}>
                                                <td><Link to={`/product-detail/${productId._id}`}>{shortenText(productId.name, 25)}</Link></td>
                                                <td><Link to={`/student-profile/${userId._id}`}>{shortenText(userId.firstName + ' ' + userId.lastName + ' - ' + userId.regNumber, 25)}</Link></td>
                                                <td>{orderCode}</td>
                                                ...
                                                <td className='icons'>
                                                    ...
                                                    <ul class="dropdown-menu drop-down-icon">
                                                        <li><Link class="dropdown-item" to={`/product-detail/${productId._id}`}><HiViewGrid size={20}/> View Item </Link></li>
